...
def visualize_trajectory_2d(pose,overall_landmarks_mu,path_name="Unknown",show_landmarks = False,show_ori=False):
    '''
    ...
    if show_landmarks:
        ax.plot(overall_landmarks_mu[0, :], overall_landmarks_mu[1, :], 'bo', markersize=1, label="Landmarks")
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.axis('equal')
    ax.grid(False)
